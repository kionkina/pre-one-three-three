class SelectionSortLarge {
  ...
  SelectionSortLarge() {
    ...
    rects = new rArrSS( temp ); //sorts
  }

  void drawSL() {
    if (counter == rects.order.size() ) { // if on last "frame"
      background(0);
      rects.drawSL(counter-1);//prints out sorted array
    } else {// if not on last "frame"
      background(0);
      rects.drawSL(counter); //iterates through "frames" one by one
      counter++;// next frame
    }
  }
